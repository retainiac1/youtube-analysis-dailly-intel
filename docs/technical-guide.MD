...
- **Reference** (§1–§4) — *how does it work?* Data model, YouTube API, the
  LLM/spend layer, and a code map.
- **Support runbook** (§5–§9) — *how do I run, operate, and fix it?* Setup,
...
`price-refresh-agent-plan.MD`). `file:line` references point at the source as of
schema **v7**.
...
daily-intel is a YouTube Shorts discovery-and-ranking pipeline plus a read-only
dashboard. A CLI pipeline (`swipefile.py`) searches the YouTube Data API for
short-form habit/health videos, filters and ranks them, and writes everything to
a single SQLite file (`data/database/swipefile.db`, WAL mode). A FastAPI dashboard
(`dashboard/app.py`) reads that file and serves a single-page app with four
pages — **board**, **trends**, **interpretation**, and **models** (a model/price
registry editor). An LLM layer (`interpret.py` +
`llm.py`) summarizes each ranked lane into a few sentences, logs every call's
tokens, and a spend panel prices those calls from a per-model price table.
...
flowchart LR
    ...
    SPA["SPA<br/>board · trends · interpretation · models"]
    LLM["interpret.py + llm.py<br/>(Anthropic/OpenAI/xAI/Google)"]
...
> `cp` of the `.db` file. See `../CLAUDE.md` and §7.

---

## 1. The data model

The schema is defined once in `SCHEMA_STATEMENTS` (`db.py:34-192`) and applied by
`init_db()` (`db.py:267-325`). Connections are opened by `get_connection()`
(`db.py:195-213`) with `PRAGMA journal_mode=WAL` and a `sqlite3.Row` factory.
...
| `models` | `model` | LLM registry: capability flags + dropdown visibility | `seed_models.py`, `/models` editor |
| `model_prices` | `id` (autoinc) | Effective-dated per-model prices for cost accounting | `seed_models.py`, `/models` editor |

Full column lists live in `db.py:34-192`. A few columns carry semantics worth
calling out:
...
4. **Schema versioning** is stamped in `PRAGMA user_version` (current
   `SCHEMA_VERSION = 7`, `db.py:32`). Additive tables use `CREATE TABLE IF NOT
   EXISTS` (idempotent, autocommitted). The two column migrations (v5
   `llm_invocations.duration_ms`; v7 `interpretations.temperature`/`seed`) and the
   v7 offline model seed run inside one explicit `BEGIN…COMMIT` so the schema and
   the version stamp move atomically — the stamp is written **last**
   (`db.py:294-320`). See §10 for the full version history.

---

## 2. YouTube API implementation

All API code is in `swipefile.py`. The client is built by `build_youtube_client()`
(`swipefile.py:65-66`): `build("youtube", "v3", developerKey=api_key)`.

### The five API calls

| Call | Function | `part` / key params | Quota |
|------|----------|---------------------|-------|
| `search.list` | `search_videos` (`swipefile.py:69-85`) | `q`, `type=video`, `videoDuration=short`, `publishedAfter=get_published_after()`, `order=viewCount`, `maxResults=RESULTS_PER_QUERY` (50), `part=snippet`, `relevanceLanguage`, optional `publishedBefore` | **100** |
| `videos.list` | `fetch_video_details` (`swipefile.py:88-95`) | `part=snippet,statistics,contentDetails,status,topicDetails` | 1 |
| `channels.list` | `fetch_channel_details` (`swipefile.py:254-280`) | batched by `CHANNEL_BATCH_SIZE` (50); `part=snippet,statistics,brandingSettings` | 1 |
| `videoCategories.list` | `fetch_video_categories` (`swipefile.py:283-306`) | `part=snippet`, `regionCode=CATEGORY_REGION` | 1 |
| `commentThreads.list` | `fetch_top_comments` (`swipefile.py:309-331`) | `maxResults=COMMENTS_PER_VIDEO` (10), `order=relevance`, `textFormat=plainText` | 1 |

Quota constants are in `config.py:223-229`. Note `search.list` does **not** pass a
`regionCode` (intentional — relevance is steered by `relevanceLanguage` instead).

### Post-processing — the filter gates

`_classify_video()` (`swipefile.py:108-144`) is the single source of truth for
acceptance. It returns the name of the **first** gate that rejects a video, or
`"kept"`. Order matters, and every field is read defensively (`.get()`) so a
**missing field fails closed** (excluded, never crashes, never passes unchecked):

1. `window` — `within_window(publishedAt, cutoff_dt)`; same horizon ranking uses.
2. `made_for_kids` — `status.madeForKids` is false.
3. `category` — `snippet.categoryId` not in `BLOCKED_CATEGORY_IDS = {"1","10","24"}` (`config.py:230`).
4. `language` — `defaultAudioLanguage` is blank **or** starts with `en` (a present non-`en` value is dropped).
5. `missing_duration` — `contentDetails.duration` present (else can't derive `is_short`).
6. `missing_views` — `statistics.viewCount` present (else can't confirm threshold).
7. `duration` — `parse_duration(...) <= SHORT_MAX_SECONDS` (180).
8. `views` — `viewCount >= MIN_VIEWS`.

`filter_videos()` (`swipefile.py:147-165`) is a thin loop over `_classify_video`
that also tallies drops by first-rejecting gate. Read-only **tuning diagnostics**
(`swipefile.py:175-244`) sit alongside: `qualifying_view_counts`,
...
`QuotaBudget` (`swipefile.py:518-545`) tracks the day: `baseline` (units already
spent today, Pacific), `cap = DAILY_QUOTA_LIMIT - SAFETY_BUFFER`, `run_units`,
`flushed_units`. `api_call_with_retry` (`swipefile.py:548-592`) checks
`can_afford` **before** each call (a proactive guard), then handles errors (see
§8). The expensive 100-unit `search.list` charge is **flushed eagerly** to
`quota_ledger` in its own committed transaction so a crash can't lose it
(`add_quota_units`, `db.py:634+`); cheaper calls are flushed once at run end
(`swipefile.py:1308-1315`).

A pre-flight guard estimates discover cost (`estimate_discover_units`,
`swipefile.py:871+`; heuristics `ESTIMATED_CHANNEL_CALLS=2`,
`ESTIMATED_COMMENT_CALLS=75`) and, if it won't fit the cap, either **downgrades**
to refresh (no flag) or **hard-stops** (explicit `--discover`).
...
flowchart TD
    ...
    CAT --> MODE{mode}
    MODE -->|discover| SEARCH[search.list per query · 100u<br/>+ videos.list · filter]
    SEARCH --> DEDUP[dedupe + accumulate matched_queries]
    DEDUP --> CH[channels.list · batched]
    CH --> CM[commentThreads.list]
    CM --> PERSIST[persist videos/channels/snapshots]
    PERSIST --> RANK[recompute rankings]
    MODE -->|refresh| RSTATS[videos.list stats only<br/>for tracked videos]
    RSTATS --> RPERSIST[persist + recompute rankings]
    RANK --> DONE([finish_run + flush quota])
    RPERSIST --> DONE
```
...
`RESULTS_PER_QUERY` (50) is a hardcoded constant (`config.py:218`), **not** a
settings.toml knob. See §6.

---

## 3. Interpretations, LLM calls, and spend
...
Four providers (`llm.py:29`): `anthropic`, `openai`, `xai`, `google`. Models are
identified everywhere by the canonical **`provider:model`** string, split at
exactly one point — `split_model()` (`llm.py:63-73`). The baseline registry and
its capability flags are seeded from `config.SEED_MODELS` (`config.py:65-74`):

| Model | `supports_temperature` | `supports_seed` | Notes |
|-------|:---:|:---:|-------|
| `anthropic:claude-haiku-4-5` | 1 | 0 | Messages API has no seed (structural) |
| `openai:gpt-5.4-nano` | 0 | 1 | reasoning model — rejects non-default temperature |
| `xai:grok-4-fast` | 1 | 1 | live-corrected by `seed_models.py` post-migrate |
...
`generate()` (`llm.py:89-121`) splits the model, normalizes the seed (a
non-positive seed → `None`; a model without `supports_seed` drops it), and
dispatches to a provider adapter (`llm.py:200-299`). Each adapter applies only the
parameters the model honors (temperature ranges: Anthropic 0–1, the others 0–2;
output capped at `MAX_OUTPUT_TOKENS = 512`) and returns a normalized
`GenerateResult` (`llm.py:50-60`): `text`, `input_tokens`, `output_tokens`,
`seed_applied` (the seed that actually governed, or `None`).
...
open window — this is what preserves the non-overlap invariant (see §9). Windows
...
(`app.py:626-810`): list/insert/update/soft-delete/restore models, add/soft-delete
price windows, and invocation-count warnings before a delete.

---

## 4. Code map — files a supporter needs to know

### Core pipeline & data (top-level `.py`)

| File | Lines | Responsibility / entry points | Touch it when… |
|------|------:|-------------------------------|----------------|
| `swipefile.py` | ~1322 | The discover/refresh pipeline: API calls, `_classify_video`, ranking, persistence, quota. CLI (`main`, `swipefile.py:1203`). | Changing discovery, filtering, ranking, or quota behavior |
| `db.py` | ~1367 | Schema, migrations, **all** SQLite read/write helpers, quota ledger, spend SQL. **Stdlib-only.** | Any schema or query change |
| `config.py` | ~467 | Tunable constants, `settings.toml` loader, `validate_config`, timestamp helpers (`now_local_iso`, `pacific_date`), `SEED_MODELS`, `DEFAULT_PRICES`. | Changing a default, a validation rule, or TZ logic |
| `interpret.py` | ~332 | Interpretation generator: prompt fields, `build_prompt`, `synthesize_lane`. CLI (`main`). | Changing prompt content or generation flow |
| `llm.py` | ~299 | Provider adapters, `split_model`, `generate`, `GenerateResult`, `LLMError`. | Adding a provider or changing param handling |
| `seed_models.py` | ~174 | Operator script: seed registry/prices + live-verify/correct xAI. | After a migration, or when an xAI model is retired |
...
| Path | Responsibility |
|------|----------------|
| `dashboard/app.py` (~614) | FastAPI app: `/api/runs`, `/api/rankings`, `/api/library`, `/api/quota`, `/api/snapshots`, `/api/rank-history`, `/api/distribution`, `/api/interpret*`, `/api/spend`, `/api/models*`, video notes/star writes. Served on loopback by `start-dashboard.sh`. Imports `config`, `db`, `interpret`, `llm` — **never `swipefile.py`** (which would drag in the YouTube API/network stack). |
| `dashboard/static/index.html` | SPA shell |
| `dashboard/static/js/router.js` | board / trends / interpretation / models routing |
...
| `…/colors.js`, `sticky-header.js`, `main.js` | shared helpers |
...
## 5. Runbook — setting up and running
...
| `PRICES` | 4 models | Seed source for `model_prices` (no longer read at display time) |
...
### What `validate_config` enforces (`config.py:340-461`)
...
non-empty slice list, numeric `efficiency_good <= efficiency_warn`).
...
by `_pick_status` (`swipefile.py:1187-1200`) in priority order:

| Status | Meaning |
|--------|---------|
| `success` | All phases completed |
| `partial` | Videos persisted, but channels/snapshots/rankings failed (non-fatal) |
...
**only** on a clean discover (`swipefile.py:1301-1302`); an aborted/partial
discover keeps it so the next run resumes cheaply. Refresh never touches it.
...
# expect: ok / 7 / a row count matching the live DB
```
...
- **LLM failures** are wrapped as `LLMError` (`llm.py:42-47`) by each adapter and
  surfaced by `POST /api/interpret` as a clean **400** with the provider-named
  message — never a raw 500. Genuine code bugs (not the SDK's API-error base) still
  surface as 500.
- **YouTube API errors** (`api_call_with_retry`, `swipefile.py:563-592`):
  `keyInvalid` or HTTP 400 → print + `sys.exit(1)` (fatal); `quotaExceeded` →
  return `None` (run aborts, work persisted); `commentsDisabled` / `forbidden` /
  `videoNotFound` → skipped silently; HTTP 429 → retry once after 5s; network
  `ConnectionError`/`TimeoutError` → retry once after 2s.
- **SQLite "database is locked"** — `run_with_db_retry` (`db.py:608-619`) retries
  3× with 0.5s backoff (a dashboard read lock is the usual cause); other
  `OperationalError`s re-raise. Writes run inside `transaction()` (`db.py:597-605`)
  which rolls back on any exception.
...
## 9. Invariants & gotchas (quick reference)

| Invariant | Where enforced |
|-----------|----------------|
| All DB timestamps are Eastern ISO-8601 + offset (UTC only for the YouTube `publishedAfter` param) | `config.now_local_iso` / `get_published_after` (`config.py:306-322`) |
| Eastern `run_date` ≠ Pacific `pacific_date`; never joined | `config.pacific_date` (`config.py:325-337`) |
| Compare timestamps parsed, never lexically | `within_window` (`swipefile.py:760`), `_parse_instant` (`db.py:1090`) |
| API-owned vs user-owned `videos` columns never clobber each other | `VIDEO_API_COLUMNS` / `VIDEO_USER_COLUMNS` (`db.py:333-345`) |
| `model_prices` windows must not overlap (overlap → double-counted cost) | `insert_price_window` auto-close (`db.py:891-914`) |
| `SHORT_MAX_SECONDS` is the single Shorts threshold (filter **and** `is_short`) | `config.py:151`, `swipefile.py` |
| Spend ignores `deleted`; dropdown honors it | `fetch_spend` vs `fetch_dropdown_models` (`db.py:744-794`, `952-1009`) |
| `llm_invocations` is append-only; current row = MAX(id) | `log_invocation`, `fetch_latest_invocation` (`db.py:492`, `733`) |
| Untagged-language videos pass the language gate (blind spot) | `untagged_audio_language_count` (`swipefile.py:207-212`) |
| Habit lane is often thin/empty | live data; use the tuning diagnostics (`swipefile.py:175-244`) to recalibrate `MIN_VIEWS`/queries |
| **Dashboard never imports `swipefile.py`** (it imports `config`, `db`, `interpret`, `llm`) | `dashboard/app.py:29-32`; shared pipeline constants live in stdlib-only `config.py` so the dashboard avoids the API/network stack |
...
### Schema version history (`db.py:10-32`)
...
| Topic | Reference |
|-------|-----------|
| Schema statements | `db.py:34-192` |
| Migrations + version stamp | `db.py:267-325` |
| Column contracts | `db.py:333-354` |
| Spend SQL | `db.py:952-1009` |
| Price window predicate | `db.py:774-794` |
| API calls | `swipefile.py:69-95`, `254-331` |
| Filter gates | `swipefile.py:108-144` |
| Quota budget + retry | `swipefile.py:518-592` |
| Run status resolution | `swipefile.py:1187-1200`, `1203-1316` |
| Prompt fields + build | `interpret.py:118-185` |
| Generate seam + adapters | `llm.py:89-121`, `200-299` |
| Config defaults + validation | `config.py:18-118`, `340-461` |
| Timestamp/TZ helpers | `config.py:306-337` |
| Dashboard routes | `dashboard/app.py:103-831` |
