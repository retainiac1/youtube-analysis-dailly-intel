...
def api_interpretation(
    ...
    """Stored interpretation text for the selected window (scope + [start_date,
    end_date]), plus the applied provenance (temperature/seed/think, NULL where not
    applicable) so the view can show how the run was produced. The window is resolved to
    the canonical window_key (None bounds = the all-time window), so the same date filter
    that drives the charts fetches the matching interpretation. Absent text renders as a
    clean empty state (status 200, empty string), never an error. The thinking CONTENT is
    not persisted, so it is absent here; it rides back only on the fresh /api/interpret
    response."""
    window_key = db.interpretation_window_key(start_date, end_date)
    row = db.run_with_db_retry(
        lambda: db.fetch_interpretation(conn, window_key, scope)
    )
    if row is None:
        return {
            "scope": scope,
...
class InterpretRequest(BaseModel):
    ...
    # Phase 2: the selected window (None bounds = all-time) and the data context. The
    # window is fed to the producers AND keys the stored row; run_date stays the active
    # run (provenance). context_mode defaults to the cheap, fixed-size aggregated path.
    start_date: str | None = None
    end_date: str | None = None
    context_mode: str = "aggregated"
...
def api_interpret(
    ...
    if body.context_mode not in interpret.INTERP_CONTEXT_MODES:
        raise HTTPException(
            status_code=422,
            detail=f"context_mode must be one of {list(interpret.INTERP_CONTEXT_MODES)}",
        )
    ...
    result = interpret.synthesize_lane(
        ...
        fields=fields, context_mode=body.context_mode,
