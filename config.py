...
# mapping live here; nothing else defines a contract integer. Codes 1 and 9 are
# shell-gate codes emitted by run-pipeline.sh BEFORE swipefile.py runs, kept OUTSIDE
# the 2..8 pipeline-classification range: 1 = "never reached the pipeline" (missing
# venv, failed backup); 9 = "another discover already in flight" (cross-process lock
# held). Neither has a swipefile _EXIT_REASON slug because swipefile never emits them.
